...
iSControl = {
    'mainDir'           :   'iS',
    'operationDir'      :   'results',
    'saveResultGlobs'   :   ['dN_ptdptdphidy.dat', '*vndata*.dat', 'v2data*'], # files in the operation directory matching these globs will be saved
...
HoTCoffeehControl = {
    ...
    'saveResultGlobs'                   :   ['all*dat', 'total*dat', 'correlfunct3D*.dat', \
                                             '*spectra.dat', 'HBT*dat', \
                                             'resonance_fraction.dat', 'chosen_resonances.dat',\
                                             'emission_density*.dat'],
}
HoTCoffeehParameters = {
...
def translate_centrality_cut():
    ...
    centrality_cut_file_name = (
        ...
        % (cut_type, model_name + nucleus_name + collision_energy,
           NNcorrelation, multiplicity_fluctuation + shapeOfEntropy
           + shapeOfNucleons)
    )
...
def hydroWithInitialCondition(aFile):
    ...
    if hydroControl['saveICFile']:
        ...
        for anotherFile in otherFilesToSave:
             copy(anotherFile, controlParameterList['eventResultDir'])
...
def doHBTWithHydroResultFiles(fileList):
	...
	#HoTCoffeehParameters['SV_resonanceThreshold'] = '0.00'
        #HoTCoffeehParameters['CF_resonanceThreshold'] = '0.00'
