...
'''
for direc in $workingDirectory/
do
	rm -rf $direc
done

for direc in $workingDirectory/job-1/event-1/post_processing_results2
do
        rm -rf $direc
done
...
(
	...
	nice -n 0 time bash ./HoTCoffeeh.sh true true \
		SV_npphi=48 chosenParticlesMode=0 qtnpts=51 delta_qz=0.02 \
		...
		qynpts=7 ignore_long_lived_resonances=1 delta_qx=0.04 delta_qy=0.04 \
	...
	mv results ../job-1/event-1/post_processing_results2
